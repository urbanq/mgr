...
public class JdbcJGPParameterDao extends SimpleJdbcDaoSupport implements JGPParameterDao {
    ...
    /**
     * join with icd10_list_code table
     */
    private final static String SELECT_BY_ICD10_CODE = "SELECT * FROM JGP_PARAMETER p JOIN icd10_list_code i on (p.list_code = i.list_code) where p.list_type=? and i.icd10_code=?";
    ...
    @Override
    public List<JGPParameter> getByRecognition(ICD10 icd10) {
        return getJdbcTemplate().query(SELECT_BY_ICD10_CODE, new Object[]{ListType.ICD10.name(), icd10.getCode()}, mapper) ;
    }
}
